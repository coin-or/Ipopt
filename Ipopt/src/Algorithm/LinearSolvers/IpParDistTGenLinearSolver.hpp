...
//                  (based on IpParDistTSymLinearSolver.hpp rev 1542)

#ifndef __IPPARDISTTGENLINEARSOLVER_HPP__
#define __IPPARDISTTGENLINEATSOLVER_HPP__
...
namespace Ipopt
{
  ...
  /** General driver for linear solvers for sparse unsymmetric
   *  matrices for a parallel Ipopt version.  The values of the matrix
   *  are collected on each processor, and on each processor the
   *  parallel linear solver is called.  So, the matrix is provided
   *  distriburedly.
   *
   ...
   *  the ParDistTGenLinearSolverInterface.
   */
  class ParDistTGenLinearSolver: public SymLinearSolver
  {
  public:
    /** @name Constructor/Destructor */
    //@{
    /** Constructor.  The solver_interface is a pointer to a linear
     *  solver for general matrices in triplet or CRC format. */
    ParDistTGenLinearSolver(SmartPtr<SparseSymLinearSolverInterface> solver_interface);

    /** Destructor */
    virtual ~ParDistTGenLinearSolver();
    //@}
     ...
     * factorization.  This method must not be called for the general
     * matrix interface.
     */
    ...
    /** Query whether inertia is computed by linear solver.  Since
     *  this interfaces looks at the matrices as unsymmetric, the
  ...
  private:
    ...
    //@{
    /** Default Constructor */
    ParDistTGenLinearSolver();

    /** Copy Constructor */
    ParDistTGenLinearSolver(const ParDistTGenLinearSolver&);

    /** Overloaded Equals Operator */
    void operator=(const ParDistTGenLinearSolver&);
    //@}
    ...
    /** Number of nonzeros of the symmetric matrix in triplet
     *  format. Note that some elements might appear multiple times in
     *  which case the values are added. */
    Index nonzeros_triplet_local_sym_;
    /** Number of nonzeros of the non-symmetric matrix in triplet
     *  format. Note that some elements might appear multiple times in
     *  which case the values are added. */
