...
IterativePardisoSolverInterface::~IterativePardisoSolverInterface()
{
   ...
   if( initialized_ )
   {
      ...
      ipfint idmy;
      double ddmy;
      F77_FUNC(pardiso, PARDISO)(PT_, &MAXFCT_, &MNUM_, &MTYPE_, &PHASE, &N, &ddmy, &idmy, &idmy, &idmy, &NRHS, IPARM_,
   ...
   )
{
   ...
   if( var != NULL )
   {
      ...
      Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Using environment OMP_NUM_THREADS = %d as the number of processors.\n",
         num_procs);
   }
#ifdef HAVE_PARDISO
...
{
   DBG_START_METH("IterativePardisoSolverInterface::MultiSolve", dbg_verbosity); DBG_ASSERT(!check_NegEVals || ProvidesInertia()); DBG_ASSERT(initialized_);
   ...
   )
{
   ...
   while( !done )
   {
      ...
      if( !have_symbolic_factorization_ )
      {
         ...
         F77_FUNC(pardiso, PARDISO)(PT_, &MAXFCT_, &MNUM_, &MTYPE_, &PHASE, &N, a_, ia, ja, &PERM, &NRHS, IPARM_, &MSGLVL_,
            &B, &X, &ERROR, DPARM_);
         if( HaveIpData() )
         {
   ...
   )
{
   ...
   while( attempts < max_attempts )
   {
      ...
      if( ERROR <= -100 && ERROR >= -110 )
      {
         ...
         if( is_normal )
         {
            Jnlst().Printf(J_WARNING, J_LINEAR_ALGEBRA, "(normal step)\n");
         }
         else
         {
            Jnlst().Printf(J_WARNING, J_LINEAR_ALGEBRA, "(PD step)\n");
         }
         PHASE = 23;
   ...
   {
      Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Termination tester requests modification of Hessian\n");
      return SYMSOLVER_WRONG_INERTIA;
   }
#if 0
   // FRANK: look at this:
...
{
   DBG_START_METH("IterativePardisoSolverInterface::NumberOfNegEVals", dbg_verbosity); DBG_ASSERT(negevals_ >= 0);
   return negevals_;
}
