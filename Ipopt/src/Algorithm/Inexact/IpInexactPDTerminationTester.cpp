...
namespace Ipopt
{
                  ...
                  Index iter, Number norm2_rhs, Number norm2_resid)
  {
    ...
    */
    ...
    Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                   " TTR: test ratio %e (norm2_rhs = %e norm2_resid = %e).\n",
                   test_ratio, norm2_rhs, norm2_resid);
    if (iter < inexact_desired_pd_residual_iter_ &&
        test_ratio > inexact_desired_pd_residual_) {
      Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                     "  TTR: immediately leaving tester with test ratio %e (norm2_rhs = %e norm2_resid = %e).\n",
                     test_ratio, norm2_rhs, norm2_resid);
    ...
    if (!compute_normal) {
      ...
      Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                     "  TCC2a testing 0.5*uWu(=%23.16e) >=\n                tcc_theta_*pow(mu,tcc_theta_mu_exponent_)*u_norm^2(=%23.16e) -->", rhs, lhs);
    }
    ...
    if (!compute_normal) {
      ...
      Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                     " TT1 testing resid_norm(=%23.16e) <=\n             tt_kappa1_*curr_tt1_norm_(=%23.16e) --> ", lhs, rhs);
      ...
      Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                     " TT1 testing rho_norm(=%23.16e) <=\n              kappa1*min(curr_tt1_norm_, last_tt1_norm_)(=%23.16e) -->", lhs, rhs);
    ...
    if (!compute_normal || tt1) {
      ...
      if (!compute_normal) {
        ...
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       " MRC testing delta_m(=%23.16e) >= max(0.5*uWu,tcc_theta_*Upsilon) +\n             sigma*nu*max(c_norm_, c_plus_Ad_norm - c_norm_)(=%23.16e) -->", rhs, lhs);
        ...
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       " MRC testing delta_m(=%23.16e) >= max(0.5*uWu,tcc_theta_*u_norm^2) +\n             sigma*nu*(c_norm_ - c_plus_Av_norm_)(=%23.16e) -->", rhs, lhs);
        tt1 = Compare_le(lhs, rhs, BasVal);
        if (tt1) {
          Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA, "satisfied\n");
        }
    ...
    if (tt3) {
      if (!compute_normal) {
        ...
        rhs = tt_kappa1_*c_norm_;
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       "  TT3 testing c_plus_Ad_norm(=%23.16e) <=\n        tt_kappa1_*c_norm(=%23.16e) -->", lhs, rhs);
        tt3 = Compare_le(lhs, rhs, BasVal);
      }
      else {
        ...
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       "  TT3 testing (c_norm_-c_plus_Ad_norm)(=%23.16e) >=\n         eps3*(c_norm_-c_plus_Av_norm_)(=%23.16e) -->", rhs, lhs);
        tt3 = Compare_le(lhs, rhs, BasVal);
      }
      if (tt3) {
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA, "satisfied\n");
    ...
    if (tt2) {
      ...
      if (!compute_normal) {
        ...
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       " TT2 testing ||gamma+A^T(y+delta)||(=%23.16e) <=\n             kappa2*curr_tt2_norm_(=%23.16e) -->", lhs, rhs);
      }
      else {
        rhs = tt_kappa2_*Min(curr_tt2_norm_, last_tt1_norm_);
        Jnlst().Printf(J_MOREDETAILED, J_LINEAR_ALGEBRA,
                       " TT2 testing ||gamma+A^T(y+delta)||(=%23.16e) <=\n             kappa2*min(curr_tt2_norm_, last_tt1_norm_)(=%23.16e) -->", lhs, rhs);
      }
