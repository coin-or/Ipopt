...
\begin{abstract}
  This document is a guide to using \Ipopt\ 3.2.0 (the C++ version
...
\item\label{it:ASLcompile} If want to compile the AMPL solver
  ...
  are instructions in {\tt ThirdParty/ASL/solvers/README}.  On my system,
  those steps are:
  \begin{enumerate}
  ...
  \item Change into the directory into which the ASL source code has
	...
	username}$\backslash$Ipopt$\backslash$ThirdParty$\backslash$ASL$\backslash$solvers}
...
\item Run the configuration script.  If you want to compile the AMPL
  ...
  \texttt{./configure CC=cl CXX=cl F77=ifort --with-asldir=ThirdParty/ASL/solvers}
