...
{\tt \$IPOPTDIR/build/Ipopt/contrib/MatlabInterface/src} folder. No changes
...
Copy that file to the {\tt \$IPOPTDIR/build/ \\ Ipopt/contrib/MatlabInterface/src}
folder then modify it as follows.
\begin{itemize}
...
from the {\tt \$IPOPTDIR/Ipopt/contrib/MatlabInterface} folder. Then, after
configuring \Ipopt, go to the folder {\tt \$IPOPTDIR/build/contrib/MatlabInterface/src}
...
to {\tt GM\_ADD\_LIBS}
\item If you would like to statically link the standard libraries into the {\tt mex} file,
add {\tt -static} to {\tt GM\_ADD\_LIBS}
\end{itemize}
...
\end{verbatim}
Alternately, you can create a folder {\tt \$IPOPTDIR/build/share}, copy the
site file to that folder, and rename it to {\tt config.site}.
...
This is possible with GCC versions 4.5.0 or newer on Mac or Windows, 4.7.3 or newer
...
interface Makefile to statically link all standard libraries into the \Ipopt { \tt mex}
...
\textbf{NOTE}: As \Matlab (version R2008a or newer) includes its own HSL MA57
