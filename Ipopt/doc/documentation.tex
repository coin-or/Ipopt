...
\item A sparse symmetric indefinite linear solver. \Ipopt\ needs
  ...
  \begin{itemize}
  \item MA27 from the Harwell Subroutine Library\\ (see \url{http://www.hsl.rl.ac.uk}).
  \item MA57 from the Harwell Subroutine Library\\ (see \url{http://www.hsl.rl.ac.uk}).
  \item MA77 from the Harwell Subroutine Library\\ (see \url{http://www.hsl.rl.ac.uk}).
  \item MA86 from the Harwell Subroutine Library\\ (see \url{http://www.hsl.rl.ac.uk}).
  ...
  The \Ipopt\ library also has mechanisms to load the MA27, MA57, MA86, and
  ...
  If you are compiling MA57, MA86, or MUMPS within the \Ipopt\ build system,
...
The \Ipopt\ distribution includes an interface for {\tt
  CUTEr}\footnote{see \url{http://cuter.rl.ac.uk/cuter-www}} (for
...
\item GAMS (modeling environment)
  ...
  includes a GAMS interface for Ipopt, see \url{https://projects.coin-or.org/GAMSlinks}.
...
\item Scilab (free Matlab-like envirnoment):
  ...
  Scilab. %needs update: \url{http://www.scilab.org/contrib/displayContribution.php?fileID=839}

  Another Scilab interface is available here: \url{http://forge.scilab.org/index.php/p/sci-ipopt}
\item Python:

  An interface to the python language is available here: \url{http://code.google.com/p/pyipopt}
\end{itemize}
...
\item Download the desired tarball from
  ...
  number, such as {\tt 3.10.2}.  There might also be daily snapshot
...
\subsubsection{Download HSL Subroutines}  %TODO: needs update
...
\begin{enumerate}
\item Go to \url{http://hsl.rl.ac.uk/archive/hslarchive.html}
...
\url{http://hsl.rl.ac.uk/hsl2007/hsl20074researchers.html}), you
...
Harwell codes, you can specify the directory with the header files and
...
{\tt \$IPOPTDIR/ThirdParty/Mumps/get.Mumps} if you have {\tt wget} installed in
your system. 
Alternatively, you can request the latest version
...
The linear solvers MA57, MA86, and MUMPS can make use of the matrix ordering
...
%TODO needs update, e.g., there is no license info on the webpage anymore...
...
%sure you read the license agreement before downloading the code. 
%Once you obtained the code and the license, please check if the version
...
MA57, MA86, MA97 or the Pardiso linear solver at runtime\footnote{This is not
...
%TODO: why Ipopt cannot be used with Pardiso on windows? still so?
...
\item Install \Ipopt \\
  ...
  \item text files {\tt ipopt\_addlibs\_cpp.txt} and {\tt
      ipopt\_addlibs\_f.txt} in \texttt{\$IPOPTDIR/build/share/coin/doc/Ipopt} that contain a
    line each with linking flags that are required for
...
\item Similarly, if you have a precompiled library containing the
  Harwell Subroutines, you can specify the directory with the
