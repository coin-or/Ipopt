...
We recommend TDM-GCC, which is available from \url{http://sourceforge.net/projects/tdm-gcc/files/TDM-GCC\%20Installer/tdm64-gcc-4.7.1-3.exe/download}.
...
\end{verbatim}
Please refer to Appendix \ref{app.options_ref} for more information on the use of these options.

Note, that the use of this feature is far from solving the complicated issue of warm-starting interior-point algorithms. As a general advice, this feature will be useful if the user observes that the solution of subsequent problems (i.e., for different data instances) preserves the same set of active inequalities and bounds (monitor the values of $z^L$ and $z^U$ for subsequent solutions). In this case, initializing the bound multipliers and setting {\tt warm\_start\_init\_point} to {\tt yes} and setting {\tt warm\_start\_bound\_push}, {\tt warm\_start\_mult\_bound\_push} and {\tt mu\_init} to a small value ($10^{-6}$ or so) will reduce significantly the number of iterations. This is particularly useful in setting up on-line applications and high-level optimization strategies in AMPL.
...
You might also want to try the adaptive barrier update (instead of the default monotone one where above we chose the initial value $10^{-6}$) when doing the warm start. This can be activated with the {\tt mu\_strategy adaptive} option, and the option {\tt mu\_oracle} gives some alternative choices. In general, the adaptive choice often leads to less iterations, but the computational cost per iteration might be higher.
