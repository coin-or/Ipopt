...
which is then modified as following.
In the {\tt maci} section (32 bit builds) or the {\tt maci64} section (64 bit 
builds), change both instances of {\tt libgfortran.dylib} to
{\tt libgfortran.a} in the {\tt FC\_LIBDIR} line and remove
{\tt -Wl,-syslibroot,\$SDKROOT} from {\tt LDFLAGS} (otherwise it can't find 
the {\tt gfortran} libraries).


The configure script of \Ipopt attempts to automatically locate the directory 
where \Matlab is installed by querying the location of the {\tt mex} 
executable. You can also manually specify the \Matlab home directory when 
calling the configure script with the flag {\tt --with-matlab-home}.
...
libraries instead. However, these static libraries need to be build in a way 
...
way that is appropriate for building an \Ipopt that is useable via the 
\Matlab. Therefore, instead of setting configure options as described in the 
previous section, it should be sufficient to setup an environment variable 
{\tt CONFIG\_SITE} that points to the site file
{\tt \$IPOPTDIR/contrib/MatlabInterface/MatlabInterface.site} before running 
configure, e.g.,
\begin{verbatim}
...
get any errors, then you should have ended up with a {\tt mex} file. On a 
Linux machine, the {\tt mex} file will be called {\tt ipopt.mexglx}.
...
\textbf{TODO:} The instructions given above are somewhat outdated due to 
recent improvements that were made for building the \Matlab interface.
...
\textbf{NOTE}: Starting with version 7.3, \Matlab can handle 64-bit 
...
can be setup to enable using this library in \Ipopt's MA57 solver interface. 
To enable this, one should specify the configure option
{\tt --enable-matlab-ma57}. Note, that using this option is not advisable to 
use this configure option if one also provides the source for MA57 via 
ThirdParty/HSL.
...
{\tt output\_file} and {\tt print\_level} options.
\begin{verbatim}
