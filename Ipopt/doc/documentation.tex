...
\url{http://www.coin-or.org/download/binary/Ipopt}.
Further, the OPTI Toolbox (\url{www.i2c2.aut.ac.nz/Wiki/OPTI}) comes with a 
...
\hyperref[opt:nlp_lower_bound_inf]{\tt nlp\_lower\_bound\_inf} will
cause \Ipopt to assume no lower bound. Likewise, specifying the upper
bound above or equal to the value of the option
\hyperref[opt:nlp_upper_bound_inf]{\tt nlp\_upper\_bound\_inf} will cause \Ipopt to
assume no upper bound.  These options,
\hyperref[opt:nlp_lower_bound_inf]{\tt nlp\_lower\_bound\_inf}
and \hyperref[opt:nlp_upper_bound_inf]{\tt nlp\_upper\_bound\_inf},
...
\item {\tt status}: (in), gives the status of the algorithm as
  ...
  \item {\tt INVALID\_NUMBER\_DETECTED}:  Algorithm received an
    ...
    also option \hyperref[opt:check_derivatives_for_naninf]{\tt check\_derivatives\_for\_naninf}.
  \item {\tt
...
if \Ipopt is run with a high value for the \hyperref[opt:print_level]{\tt print\_level} 
...
\hyperref[opt:nlp_upper_bound_inf]{\tt nlp\_upper\_bound\_inf} as infinity.
The default value of \hyperref[opt:nlp_lower_bound_inf]{\tt nlp\_lower\_bound\_inf}
and \hyperref[opt:nlp_upper_bound_inf]{\tt nlp\_upper\_bound\_inf} is
...
\hyperref[opt:output_file]{\tt output\_file} and
\hyperref[opt:print_level]{\tt print\_level} options.
\begin{verbatim}
...
\hyperref[opt:derivative_test]{\tt derivative\_test}.
...
by the option \hyperref[opt:derivative_test_perturbation]{\tt derivative\_test\_perturbation}.  The default value
...
\hyperref[opt:derivative_test_tol]{\tt derivative\_test\_tol}.
...
\hyperref[opt:derivative_test_print_all]{\tt derivative\_test\_print\_all}
option to {\tt yes}.
...
\hyperref[opt:derivative_test_first_index]{\tt derivative\_test\_first\_index}
...
feature by setting the \hyperref[opt:hessian_approximation]{\tt hessian\_approximation} 
...
\hyperref[opt:warm_start_init_point]{\tt warm\_start\_init\_point} \\
\hyperref[opt:warm_start_bound_push]{\tt warm\_start\_bound\_push} \\
\hyperref[opt:warm_start_mult_bound_push]{\tt warm\_start\_mult\_bound\_push}
...
setting \hyperref[opt:warm_start_init_point]{\tt warm\_start\_init\_point} to {\tt yes} and 
setting \hyperref[opt:warm_start_bound_push]{\tt warm\_start\_bound\_push},
\hyperref[opt:warm_start_mult_bound_push]{\tt warm\_start\_mult\_bound\_push} and
\hyperref[opt:mu_init]{\tt mu\_init} to a small value ($10^{-6}$ or so) will reduce 
...
You might also want to try the adaptive barrier update (instead of the default monotone one where above we chose the initial value $10^{-6}$) when doing the warm start. This can be activated by setting the \hyperref[opt:mu_strategy]{\tt mu\_strategy} option to {\tt adaptive}. Also the option \hyperref[opt:mu_oracle]{\tt mu\_oracle} gives some alternative choices. In general, the adaptive choice often leads to less iterations, but the computational cost per iteration might be higher.
...
\hyperref[opt:print_options_documentation]{\tt print\_options\_documentation}{\tt~yes}
\medskip
...
default setting for \hyperref[opt:print_level]{\tt print\_level}. The output is designed to
...
\item[{\tt inf\_pr}:] The unscaled constraint violation at the current
  ...
  at the current point.  The option \hyperref[opt:inf_pr_output]{\tt inf\_pr\_output} can
...
\item[{\tt Solved\_To\_Acceptable\_Level}:]  $\;$ \\
  ...
  \hyperref[opt:acceptable_tol]{\tt acceptable\_*} options.
...
\item[{\tt Diverging\_Iterates}:]  $\;$ \\
  ...
  \hyperref[opt:diverging_iterates_tol]{\tt diverging\_iterates\_tol}.
...
\item[{\tt Maximum\_Iterations\_Exceeded}:]  $\;$ \\
  ...
  iterations as specified by the option \hyperref[opt:max_iter]{\tt max\_iter}.
\item[{\tt Maximum\_CpuTime\_Exceeded}:]  $\;$ \\
  Console Message: {\tt EXIT: Maximum CPU time exceeded.} \\
  This indicates that \Ipopt has exceeded the maximum number of
  CPU seconds as specified by the option \hyperref[opt:max_cpu_time]{\tt max\_cpu\_time}.
...
\item[{\tt Error\_In\_Step\_Computation}:]  $\;$ \\
  ...
  \hyperref[opt:check_derivatives_for_naninf]{\tt check\_derivatives\_for\_naninf} option.
...
\item[{\tt Not\_Enough\_Degrees\_Of\_Freedom}:]  $\;$ \\
  ...
  variables by default, see also the \hyperref[opt:fixed_variable_treatment]{\tt fixed\_variable\_treatment} option).
...
the options \hyperref[opt:print_info_string]{\tt print\_info\_string}
to \texttt{yes}. With
...
\hyperref[opt:print_options_documentation]{\tt print\_options\_documentation}
