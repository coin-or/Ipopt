...
All available interface methods are documented in the {\tt javadoc} 
documentation of the {\tt Ipopt} class, which is currently available at 
\url{http://www.vrtech.com.br/pub/jipoptdoc}.
...
In the class constructor, we setup arrays with variable bounds and constraint 
sides and call the {\tt create()} method of \JIpopt:
...
public HS071() {
  ...
  /* set the values of the constraint sides */
  g_L[0] = 25;
  g_U[0] = 2e19;
  g_L[1] = 40;
  g_U[1] = 40;
...
\noindent Further, we add an auxiliary method that returns the initial values for the 
primal variables:
\begin{verbatim}
...
routines creates an instance of our object and calls the solve method with the 
initial guess.
\begin{verbatim}
...
\end{verbatim}
The solve method returns the \Ipopt solve status as integer, which can be used 
to check whether the problem was solved successfully. Further, it returns 
primal solution values for the variables in the argument array {\tt x}.
API functions like {\tt getObjValue()}, {\tt getMultConstraints()}, {\tt 
getMultLowerBounds()}, and {\tt getMultUpperBounds()} can be used to obtain 
the objective value and dual solution values.
