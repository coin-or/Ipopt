...
\item[{\tt inf\_pr}:] The scaled primal infeasibility at the current
  point. This quantities measure the infinity-norm (max) of the
  internal equality constraints, Eq.~(4b) in the implementation paper
  ...
  phase, this value is the primal infeasibility of the original
  problem at the current point.
...
the option ``\texttt{print\_info\_string}'' to ``\texttt{yes}''.
Then, during the corresponding operation, a tag will appear at the end
of an iteration line with the following diagnostic meaning. These tags
correspond to the basic \Ipopt\ code. They are useful to flag
difficulties for a particular \Ipopt\ run.
