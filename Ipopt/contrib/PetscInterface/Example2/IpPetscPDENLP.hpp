...
//
// $Id: IpLibMeshPDENLP.hpp 1887 2011-01-28 03:17:57Z andreasw $
//
...
namespace Ipopt
{
  ...
  class PetscPDENLP : public ParTNLP
  {
  public:
    /**@name Constructors/Destructors */
    //@{
    PetscPDENLP(PetscPDEBase& PetscPDE,
                  Journalist& jnlst);
  ...
  private:
    ...
    /** Copy Constructor */
    PetscPDENLP(const PetscPDENLP&);

    /** Overloaded Equals Operator */
    void operator=(const PetscPDENLP&);
    //@}

    /** Object containing all information regarding the PDE */
    PetscPDEBase* PetscPDE_;
    int control_first_;
    int control_last_;
    int state_first_;
    int state_last_;
    ...
    void optim_var_libMesh2local(Vec& state, Vec& control, Number* plocalDest);

    void optim_var_global2libMesh(const Number* pglobal, Vec& state, Vec& control);
