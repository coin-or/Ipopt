...
First, a file \texttt{Makevars} in the source directory of the interface package needs to be configured for your system from the file \texttt{Makevars.in}. The \texttt{configure\_ipoptr.R} script should make this easier. Start R, go to the directory where the \texttt{ipoptr} interface is located (I assume \texttt{~/Ipopt/contrib/RInterface}) and load the script. In the code below, you should change the directories to the correct ones for your system.
<<configureIpoptrStep1, eval=FALSE>>=
setwd('~/Ipopt/contrib/RInterface')
...
install.packages('~/Ipopt/contrib/RInterface', repos=NULL, type='source')
@
