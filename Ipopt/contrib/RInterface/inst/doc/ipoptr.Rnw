...
\section{Using data}
The final subject we have to cover, is how to pass data to an objective function or the constraints. This is achieved by writing a wrapper function around the objective function with the parameters as an argument, or by defining an environment that holds the data, and passing this environment to \texttt{ipoptr}. Both methods are shown in \texttt{tests/parameters.R}, here I will only show the second. 
...
\]
for different values of the parameters $a_1$, $a_2$ and $a_3$. First we define the objective function and its gradient using, assuming that there is some variable \texttt{params} that contains the values of the parameters.
<<defineDataFunction>>=
eval_f <- function(x) { 
...
@
Then we define an environment that contains specific values of \texttt{params}
<<defineDataEnvironment>>=
# define a new environment that contains params
auxdata        <- new.env()
auxdata$params <- c(1,2,3)
@
In this case the values of the parameters are $a_1=1$, $a_2=2$ and $a_3=3$. To solve this we supply \texttt{auxdata} as an argument to \texttt{ipoptr}, that will take care of evaluating the functions in the correct environment, so that auxiliary data is available.
<<solveDataEnvironment>>=
