...
namespace Ipopt
{
  ...
  void CompoundSymMatrix::PrintImpl(FILE* fp, std::string name, Index indent, std::string prefix) const
  {
    fprintf(fp, "\n");
    for (Index ind=0; ind<indent; ind++) {
      fprintf(fp, " ");
    }
    fprintf(fp, "%sCompoundSymMatrix \"%s\" with %d rows and columns components:\n",
    ...
    for (Index irow = 0; irow < NComps_Dim(); irow++ ) {
      for (Index jcol = 0; jcol <= irow; jcol++ ) {
        ...
        if (ConstComp(irow,jcol)) {
          ...
          fprintf(fp, "%sThis component has not been set.\n", prefix.c_str());
        }
      }
    }
  }
