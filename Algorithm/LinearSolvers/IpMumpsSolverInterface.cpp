...
extern "C"
   {
   #include "\ipopt-3.0.1\Extern\MUMPS_4.6.1\libseq\mpi.h"
   }
...
ESymSolverStatus MumpsSolverInterface::InitializeStructure(Index dim, Index nonzeros,
   ...
   mumps_data.a = a;
   mumps_data.irn = (Index*)ia;
   mumps_data.jcn = (Index*)ja;
   mumps_data.n = n;
   mumps_data.nz = nz;
   ...
   while (trycount < 5)
      ...
      if (error < 0)
         ...
         if (error == -8 || error == -9)//not enough memory
            ...
            mumps_data.icntl[13] = 2.0 * mem_percent;
            trycount++;
            continue;
            }
         }
