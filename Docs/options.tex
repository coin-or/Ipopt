...
\paragraph{diverging\_iterates\_tol:} Threshold for maximal value of primal iterate. $\;$ \\
...
\begin{itemize}
   ...
   \item wsmp: use WSMP package (not yet working)
...
scaling factors for the augmented system. This
scaling is independent of the NLP problem scaling.
...
algorithm will starting the scaling method only
when the solutions to the linear system seem not
good, and then use it until the end.
The default value for this string option is "yes".
\\ 
Possible values:
\begin{itemize}
   \item no: Always scaling the linear system.
   \item yes: Start using linear system if solutions seem not
good.
\end{itemize}
...
space is used.  However, choosen a larger number
...
\paragraph{wsmp\_num\_threads:} Number of threads to be used in WSMP $\;$ \\
 This determine on how many SMP CPUs WSMP is
running on. The valid range for this integer option is
...
IPARM(16). The valid range for this integer option is
...
\paragraph{wsmp\_scaling:} Toggle for swiching on WSMP's internal scaling. $\;$ \\
 Yes corresponds to setting WSMP's IPARM(10) to 1,
otherwise this is set to 0.
The default value for this string option is "yes".
\\ 
Possible values:
\begin{itemize}
   \item no: don't use expensive scaling
   \item yes: use expensive scaling
\end{itemize}

\paragraph{wsmp\_singularity\_threshold:} WSMP's singularity threshold. $\;$ \\
