...
trademarks: AMPL, IBM, Intel, Matlab, Microsoft, Visual Studio C++,
Visual Studio C++ .NET
\end{small}
...
{\bf ToDo: Add information about the Wiki pages here!}
...
\item Download the latest tarball from
  ...
  getting the correct file; it is localted towards the bottom of that
...
Yet another note: If you have a precompile library containing the
...
\item {\tt status}: (in), gives the status of the algorithm as
  ...
  \item {\tt USER\_REQUESTED\_STOP}: The user call-back function {\tt
    ...
    returned {\tt flase}, i.e., the user code requested a premature
  ...
  \item {\tt ERROR\_IN\_STEP\_COMPUTATION}: An unrecoverable error
    occured while \Ipopt\ tried to compute the search direction.
...
inialize that object, process this options (particularly the output
relatex options), and the call to {\tt app->OptimizeTNLP(...)}  will
...
occured, and the program is to be aborted.

The value returned in {\tt obj\_scaling} determines, how \Ipopt\
should interally scale the objective function.  For example, if this
...
feature using the {\tt hessian\_information} and {\tt
