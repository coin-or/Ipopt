...
\item\label{it:ASLcompile} If want to compile the AMPL solver
  ...
  \item Change into the directory where the ASL source code has been
    downloaded, e.g.,
...
\item Now you need to make sure that Cygwin knows about the native
  ...
  or whatever you used in \ref{eq:intelsetup}.

\item Now open a new Cygwin window and change into the directory where
  you put the \Ipopt\ source code (where the {\tt configure} file is)

\item Now run the configuration script.  If you want to compile the
  AMPL solver executable (i.e., you followed the instruction in
  ...
  want to generate the \Ipopt\ library, just type
