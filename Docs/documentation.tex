...
\begin{abstract}
  This document is a guide to using \Ipopt\ 3.1.0 (the new C++ version
...
%       http://www-users.cs.umn.edu/\verb|~|agupta/wsmp.html})
  ...
  order to run \Ipopt.  Currently, there is development on integrating
  also MUMPS, TAUCS, and WSMP by contributors, but this work has not
  yet been completed.
...
user can edit the webpages from the regular web browser.  {\bf In
  ...
  password; simply follow the instructions in the last paragraph of
  the Documentation section on the main project page.}
...
\item Download the latest tarball from
  ...
  ``\texttt{3.x.x.}'' is the version number).  Make sure you are
  getting the correct file; it is located toward the bottom of that
  page; don't confuse it with the tarball of the old (Fortran) \Ipopt\
  version ({\tt Ipopt\_{\em date}.tgz}).  Put this file in a directory
  under which you want to put the \Ipopt\ installation.
...
Library), BLAS, LAPACK.  It also requires at least one sparse
...
\subsubsection{Other Linear Solvers}

If you would like to compile \Ipopt\ with another linear solver, you
need to obtain the corresponding library on your own and, if
necessary, compile it yourself if it is not available in precompiled
form.
...
\item If you want, you can run a short test to verify that the
  compilation and installation was successful.  For this, you just
  enter\\
  {\tt \$ make test}\\
...
There are three ways to install \Ipopt\ on Windows systems.  The first
...
\item\label{it:ASLcompile} If want to compile the AMPL solver
  executable, you need to compile the ASL library ``by hand''.  There
  are instructions in {\tt Extern/ASL/solvers/README}.  On my system,
  those steps are:
  \begin{enumerate}
  \item Open a DOS prompt window.
  \item\label{it:ASLcompileSetup} Run the Intel Fortran compiler setup
    script, so that the compiler executables are in the path.  On my
    system, I need to run

  \begin{equation}\label{eq:intelsetup}
  ...
  \end{equation}
    ...
    (Consult your compiler documentation if that doesn't work for you.)
  \item Change into the directory into which the ASL source code has
    been downloaded, e.g.,

    \texttt{cd $\backslash$cygwin$\backslash$home$\backslash${\em
	username}$\backslash$Ipopt$\backslash$Extern$\backslash$ASL$\backslash$solvers}

  \item Copy the file {\tt makefile.vc} to {\tt makefile}

    \texttt{copy makefile.vc makefile}

  \item Copy the file {\tt details.c0} to {\tt details.c}

    \texttt{copy details.c0 details.c}

    and edit the file {\tt details.c} to change the string to
    something meadingful like ``{\tt MS VC++ .NET 2003}''.

  \item Compile the library usgin {\tt nmake}

    \texttt{nmake}

  \item Rename the generated library to {\tt amplsolver.a}

    \texttt{ren amplsolv.lib amplsolver.a}

  \item Close the DOS Window
  \end{enumerate}
...
\item Now you need to make sure that Cygwin knows about the native
  ...
  Here you need need to add the line

  \texttt{call ``C:$\backslash$Program
    Files$\backslash$Intel$\backslash$Fortran$\backslash$compiler80$\backslash$IA32$\backslash$BIN$\backslash$ifortvars.bat''}

  or whatever you used in (\ref{eq:intelsetup}) above.

\item Open a new Cygwin window and change into the directory where you
  put the \Ipopt\ source code (where the {\tt configure} file is)

\item Run the configuration script.  If you want to compile the AMPL
  solver executable (i.e., you followed the instruction in
  Step~\ref{it:ASLcompile} above), you type

  \texttt{./configure CC=cl CXX=cl F77=ifort --with-asldir=Extern/ASL/solvers}

  If you don't want to compile the AMPL solver executable and just
  want to generate the \Ipopt\ library, type only

  \texttt{./configure CC=cl CXX=cl F77=ifort}

  Make sure the last message is

  \texttt{Configuration successful}

\item Now you can compile the code with

  \texttt{make install}

  and, if desired, test the installation with {\tt make test}
...
Extern$\backslash$blas}, {\tt Extern$\backslash$lapack}, and {\tt
Extern$\backslash$HSL} directories---ASL is not required for the
...
\texttt{\$IPOPTDIR$\backslash$Windows$\backslash$VisualStudio\_dotNET}
...
your own source code in the {\tt Examples} subdirectory.
...
The {\tt Examples/ScalableProblems} directory contains other NLP
problems coded in C++.
...
\item Similarly, if you have a precompiled library containing the
  ...
  \verb|--with-asldir|.  If you want to compile \Ipopt\ with the
  linear solver Pardiso\footnote{see {\tt
      http://www.computational.unibas.ch/cs/scicomp/software/pardiso/}},
  you need to specify the location of the library with the
  \verb|--with-pardiso| flag.
