...
\Ipopt\ algorithm).  By default, MC19 only used to scale the linear
...
you obtained the code and the license, please check if there are
bugfix releases of the WSMP library available at

{\tt http://www-users.cs.umn.edu/\verb|~|agupta/wsmp}

You can use those bugfix releases with the license you obtained from
alphaWorks.
...
\item\label{it:ASLcompile} If want to compile the AMPL solver
  ...
  \item Copy the file {\tt details.c0} to {\tt details.c}
    ...
    something meaningful like ``{\tt MS VC++ .NET 2003}''.

  \item Compile the library using {\tt nmake}
