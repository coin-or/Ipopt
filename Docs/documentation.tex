...
set for the {\tt configure} script by typing {\tt configure --help}.
...
\item The {\tt configure} script tries to determine automatically, if
  ...
  library) explicitly, using the {\tt --with-blas} flag for {\tt
    configure}.  For example,

  {\tt ./configure --with=blas="-L \$HOME/lib -latlas"}
...
\item Similarly, if you have a precompiled library containing the
  Harwell Subroutines, you can specify its location with the {\tt
    --with-hsl} flag.
...
\item It is possible to compile the \Ipopt\ library in a debug
  configuration, by specifying {\tt --enable-debug}.  Then the
